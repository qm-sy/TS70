...
void  Sw_Data_Send()
{
	uint16_t  Va=Read_Dgus(0x0f00);
	uint16_t  V1=Read_Dgus(0x0f01);
	uint16_t addr_vol;
	uint8_t  val_H;
	uint8_t  val_L;
	...
	{	   
		uint8_t i = 0;
		uint16_t Temp = 0;
		uint8_t Val[100] = {0}; 	   //5A A5 06 83 ADDR Len XX XX 
...
/****************************数据自动上传************************************************/
void uart_data_send(uint8_t* Arr,uint8_t Uartnum,bit Outo_send,bit Crc_ck )
{
	uint8_t i = 0;      
