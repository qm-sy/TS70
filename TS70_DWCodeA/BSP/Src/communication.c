...
void Uart5_Send_Statu_Init( void )
{
    ...
    rs485_5.TX_buf[1024] = 0;
    rs485_5.RX_buf[1024] = 0;
...
{   
    ...
    if( SCON3T & S5TI )          //
    {
        ...
        SCON3T &= ~S5TI;     
        /* 3, 依次将TX_buf中数据送出（写SBUF0操作即为发送）    */
        if( rs485_5.TX_send_bytelength != 0 )
        {
            SBUF3_TX = rs485_5.TX_buf[rs485_5.TX_send_cnt++];
            rs485_5.TX_send_bytelength--;
        }else
        {
            rs485_5.TX_send_cnt = 0;
            TR5 = 0;
...
{   
    ...
    if( SCON3R & S5RI )
    {
        ...
        {
            /* 4, 数据包大于RX_buf 则从头计数                  */
            if( rs485_5.RX_rcv_cnt > 1024 )
            {
                rs485_5.RX_rcv_cnt = 0;
            }

            /* 5, 依次将RX_buf中数据接收（读S2BUF操作即为接收）*/
            rs485_5.RX_buf[rs485_5.RX_rcv_cnt] = SBUF3_RX;
            rs485_5.RX_rcv_cnt++;
        }
        /* 6, 重置接收完毕判断时间                              */
...
void Tim1_Isr( void ) interrupt 3
{
    ...
    {
        rs485_5.RX_rcv_timeout--;
        /* 2, 如果接收超时                                          */
        if( rs485_5.RX_rcv_timeout == 0 )  
        {
            if( rs485_5.RX_rcv_cnt > 0 )  
            {   
                 /* 3, 接收完毕标志位亮起并初始化接收缓冲区         */
                rs485_5.RX_rcv_end_Flag = 1;    
            }
        }
    } 
}
