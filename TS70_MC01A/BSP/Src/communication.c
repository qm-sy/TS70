...
void Uart4_Send_Statu_Init( void )
{
    ...
    rs485_4.send_buf[128] = 0;
    rs485_4.rcv_buf[128] = 0;
    rs485_4.send_bytelength = 0;
    rs485_4.send_cnt = 0;
    rs485_4.rcv_timeout = 0;
    rs485_4.rcv_cnt = 0;
    DR4_485 = 0;
}
...
{   
    ...
    if( S4CON & S4TI )          //
    {
        ...
        if( rs485_4.send_bytelength != 0 )
        {
            S4BUF = rs485_4.send_buf[rs485_4.send_cnt++];
            rs485_4.send_bytelength--;
        }else
        {
            rs485_4.send_cnt = 0;
            DR4_485 = 0;
    ...
    if( S4CON & S4RI )
    {
        ...
        /* 3, 判断数据包是否接收完毕                           */
        if( !rs485_4.rcv_end_flag )
        {
            /* 4, 数据包大于RX_buf 则从头计数                  */
            if( rs485_4.rcv_cnt > 128 )
            {
                rs485_4.rcv_cnt = 0;
            }

            /* 5, 依次将RX4_buf中数据接收（读S2BUF操作即为接收）*/
            rs485_4.rcv_buf[rs485_4.rcv_cnt] = S4BUF;
            rs485_4.rcv_cnt++;
        }
        /* 6, 重置接收完毕判断时间                              */
...
{
    /* 1, 如果接收未超时                                             */
    if ( rs485_4.rcv_timeout != 0 )  
    {
        rs485_4.rcv_timeout--;
        /* 2, 如果接收超时                                          */
        if( rs485_4.rcv_timeout == 0 )  
        {
            if( rs485_4.rcv_cnt > 0 )  
            {   
                 /* 3, 接收完毕标志位亮起并初始化接收缓冲区         */
                rs485_4.rcv_end_flag = 1;    
            }
        }
    } 
}
