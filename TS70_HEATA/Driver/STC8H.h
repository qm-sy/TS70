...
//      char dat;
//
//      READ_USB(CSR0, dat);        //??USB?????
//      WRITE_USB(FADDR, 0x00);     //��USB?????
...
#define	EAXSFR()		P_SW2 |= 0x80		/* MOVX A,@DPTR/MOVX @DPTR,A???????????????SFR(XSFR) */
#define	EAXRAM()		P_SW2 &= ~0x80		/* MOVX A,@DPTR/MOVX @DPTR,A???????????????RAM(XRAM) */
